...
//        });
    ...
    public void loadDataSuccess(MonthListBean monthListBean) {
        ...
        String t_outcome = monthListBean.getT_outcome();
        String t_income = monthListBean.getT_income();
        tOutcome.setText(t_outcome);
        tIncome.setText(t_income);
        tTotal.setText(monthListBean.getT_total());
        float out = Float.parseFloat(t_outcome);
        float in = Float.parseFloat(t_income);
        if (out > in) {
            ToastUtils.show(getActivity(), "您本月支出已超出收入！");
        }
        initTitle(in);
    }
    ...
    //3000到10000显示普通家庭
    //10000往上就是高薪家庭
    private void initTitle(float in) {
        if (in < 3000.0) {
            title.setText("低薪家庭");
        } else if (in >= 3000.0 && in <= 10000.0) {
            title.setText("普通家庭");
        } else if (in > 10000.0) {
            title.setText("高薪家庭");
        }
